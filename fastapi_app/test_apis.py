...
def test_export_extraction():
    ...
    try:
        headers = {"Authorization": "Bearer mock-token"}
        response_csv = httpx.post(f"{BASE_URL}/api/v1/export?format=csv", json={"extraction_id": extraction_id}, headers=headers) # Corrected path
    ...
    try:
        headers = {"Authorization": "Bearer mock-token"}
        response_excel = httpx.post(f"{BASE_URL}/api/v1/export?format=excel", json={"extraction_id": extraction_id}, headers=headers) # Corrected path
...
def run_tests():
    ...
    output_filename = "fastapi_app/test_results.txt"
    try:
